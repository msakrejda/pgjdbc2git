...
if [ ! -f "$tar_file" ]
then
    echo "Expected path to PostgreSQL JDBC CVS export as argument; got '$tar_file'"
    exit 1
fi
...
rm -rf cvs-repo pgjdbc.git pgjdbc-checkout cvs2svn-tmp
...
# Fix up the REL6_4 branch, which cvs2git puts in the wrong place (it's easier to fix here than in CVS):
cd pgjdbc-checkout
